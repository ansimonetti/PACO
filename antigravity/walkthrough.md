...
- **Pre-Condition**: Input `(A,B)`. Graph area empty.
...
- **Root Cause**: `src/paco/parser/bpmn_parser.py` was incorrectly slicing impact lists using the Horizon (`h`) parameter (`impact[0:len-h]`), resulting in empty impacts for scalar inputs when `h > 0`.
- **Fix Applied**: Modified `bpmn_parser.py` to use `impact` directly (scalar/vector as provided). Confirmed fix via manual debug script logic.
- **Verification**: 
  - LLM successfully generates expression `(A,B)`.
